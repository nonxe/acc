 ...
 *  ---------------------------
 *  GSM Module (SIM800L):
 *    GSM TX  ->  Arduino UNO Pin 2 (rxPin - SoftwareSerial RX)
 *    GSM RX  ->  Arduino UNO Pin 3 (txPin - SoftwareSerial TX)
 *
 *  GPS Module (NEO-6M):
 *    GPS TX  ->  Arduino UNO Pin 8 (AltSoftSerial RX - fixed pin)
 *    GPS RX  ->  Arduino UNO Pin 9 (AltSoftSerial TX - fixed pin)
 *
 *  Accelerometer (ADXL335):
 *    X-axis  ->  Arduino UNO Pin A1
 *    Y-axis  ->  Arduino UNO Pin A2
 *    Z-axis  ->  Arduino UNO Pin A3
 *
 *  I2C LCD (16x2):
 *    SDA     ->  Arduino UNO Pin A4
 *    SCL     ->  Arduino UNO Pin A5
 *
 *  Buzzer:
 *    Signal  ->  Arduino UNO Pin 12
 *
 *  Cancel Button:
 *    Signal  ->  Arduino UNO Pin 11 (INPUT_PULLUP)
 *
 * =========================================================================
 */
...
//--------------------------------------------------------------
//GSM Module RX pin to Arduino UNO Digital Pin 3
//GSM Module TX pin to Arduino UNO Digital Pin 2
#define rxPin 2
#define txPin 3
SoftwareSerial sim800(rxPin,txPin);
//--------------------------------------------------------------
//GPS Module connected via AltSoftSerial
//AltSoftSerial uses FIXED pins on Arduino UNO (ATmega328P):
//  RX = Pin 8  (connect to GPS TX)
//  TX = Pin 9  (connect to GPS RX)
...
#define BUTTON 11
//--------------------------------------------------------------
//ADXL335 Accelerometer Analog Pins on Arduino UNO
#define xPin A1
#define yPin A2
#define zPin A3
//--------------------------------------------------------------
...
{
  //--------------------------------------------------------------
  //Serial.println("Arduino UNO serial initialize");
  Serial.begin(9600);
  //--------------------------------------------------------------
  ...
  //when you reset your Arduino UNO. (By pressing reset button)
